...
p_POS_reg <- ggplot() +
 ...
 scale_y_continuous(name="Taux de Ø") +
	...
	annotate("text", label=paste("Empan des régressions : 0.5",
	         ...
	         x = 1350, y = 0.5, hjust = 0, vjust = 1) +
	annotate("rect", xmin = 1110, xmax = 1135, ymin = 0.535, ymax = 0.585, colour="black", fill=NA) +
	 annotate("text", label = "1", x = 1122.5, y = 0.595, hjust = 0.5, vjust = 0) +
	annotate("rect", xmin = 1200, xmax = 1220, ymin = 0.7, ymax = 0.75, colour="black", fill=NA) +
	 annotate("text", label = "2", x = 1210, y = 0.76, hjust = 0.5, vjust = 0) +
	annotate("rect", xmin = 1302, xmax = 1322, ymin = 0.475, ymax = 0.6, colour="black", fill=NA) +
	 annotate("text", label = "3", x = 1312, y = 0.61, hjust = 0.5, vjust = 0)
	# annotate("rect", xmin = 1302, xmax = 1322, ymin = 0.475, ymax = 0.6, colour="black", fill=NA) +
	#  annotate("text", label = "4", x = 1312, y = 0.61, hjust = 0.5, vjust = 0)

POS__V_valley1 <- POS__V %>% filter(datecomposition > 1110) %>%
                             filter(datecomposition < 1138)
POS__V_pike1   <- POS__V %>% filter(datecomposition > 1199) %>%
                             filter(datecomposition < 1221)
POS__V_valley2 <- POS__V %>% filter(datecomposition > 1301)
...
chisq.test(pike1_table)
 pike1_chi_df <- chisq.test(pike1_table)$parameter
	...
	pike1_chi_p <- format.pval(pv = pike1_chi_p,
		                          ...
		                          eps = 0.0001, # eps = the threshold value above wich the function will replace the pvalue by "<0.0xxx"
