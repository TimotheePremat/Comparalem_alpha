...
#compute stats
	...
	E_vs_nonE_t <- t.test(POS_E_$Tx_POS__V,POS_nonE_$Tx_POS__V)
	 E_vs_nonE_t <- t.test(POS_E_$Tx_POS__V,POS_nonE_$Tx_POS__V)$statistic
		...
		E_vs_nonE_p <- t.test(POS_E_$Tx_POS__V,POS_nonE_$Tx_POS__V)$p.value
			E_vs_nonE_p <- if (E_vs_nonE_p <0.005) {
					                 ("< 0.005")
					                } else if (E_vs_nonE_p <0.05) {
					                 ("< 0.05")
					                } else
					                 (paste("=",round(E_vs_nonE_p, digits=3)))
	E_vs_nonE_t_paired <- t.test(POS_nonE_OVW_in_E$Tx_POS__V,POS_E_OVW_in_nonE$Tx_POS__V,paired=TRUE)
	 ...
	 E_vs_nonE_paired_p <- t.test(POS_nonE_OVW_in_E$Tx_POS__V,POS_E_OVW_in_nonE$Tx_POS__V,paired=TRUE)$p.value
			E_vs_nonE_paired_p <- if (E_vs_nonE_paired_p <0.005) {
					                 ("< 0.005")
					                } else if (E_vs_nonE_paired_p <0.05) {
					                 ("< 0.05")
					                } else
					                 (paste("=",round(E_vs_nonE_paired_p, digits=3)))
	##Depending on following context
	V_vs_C_t <- t.test(POS__V$Tx_POS_EnonE,POS__C$Tx_POS_EnonE)
	 V_vs_C_t <- t.test(POS__V$Tx_POS_EnonE,POS__C$Tx_POS_EnonE)$statistic
		...
		V_vs_C_p <- t.test(POS__V$Tx_POS_EnonE,POS__C$Tx_POS_EnonE)$p.value
			V_vs_C_p <- if (V_vs_C_p <0.005) {
																						("< 0.005")
																					} else if (V_vs_C_p <0.05) {
																						("< 0.05")
																					} else
																						(paste("=",round(V_vs_C_p, digits=3)))
	V_vs_C_t_paired <- t.test(POS__V_OVW_in_C$Tx_POS_EnonE,POS__C_OVW_in_V$Tx_POS_EnonE,paired=TRUE)
	 ...
	 V_vs_C_paired_p <- t.test(POS__V_OVW_in_C$Tx_POS_EnonE,POS__C_OVW_in_V$Tx_POS_EnonE,paired=TRUE)$p.value
			V_vs_C_paired_p <- if (V_vs_C_paired_p <0.005) {
																						("< 0.005")
																					} else if (V_vs_C_paired_p <0.05) {
																						("< 0.05")
																					} else
																						(paste("=",round(V_vs_C_paired_p, digits=3)))
...
##Depending on word ending
	...
	g_POS_E_vs_nonE_ALL <- ggarrange(p_POS_E_arrange,
		         p_POS_nonE_arrange,
		         p_POS_E_w_arrange,
	          ...
	          nrow = 3,
										 align = "v")
	g_POS_E_vs_nonE_ALL <- annotate_figure(g_POS_E_vs_nonE_ALL,top =
		text_grob(paste(my_var2), face = "bold", size = 14))
...
##Depending on following context
	...
	g_POS_V_vs_C_ALL <- ggarrange(p_POS__Carrange,
	          p_POS__Varrange,
											p_POS__C_w_arrange,
											p_POS__V_w_arrange,
											Overview_V_vs_C,
	          ...
	          nrow = 3,
										 align = "v")
 g_POS_V_vs_C_ALL <- annotate_figure(g_POS_V_vs_C_ALL,top =
		text_grob(paste(my_var2), face = "bold", size = 14))
